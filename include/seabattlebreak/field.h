...
class Field {
 public:
  enum class CellState { kUnknown = 0, kEmpty, kShip, kWreck };
  ...
  CellState cell_state(int x, int y) const { return field_[x][y]; }
  CellState cell_state(Coord p) const { return field_[p.x][p.y]; }
